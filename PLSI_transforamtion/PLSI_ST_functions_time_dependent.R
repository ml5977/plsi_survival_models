...
loglik.fun <- function(final.gamma, final.alpha, final.q, Z, indY, Delta, n, rho, r){
  coef.final <- c(final.gamma, final.alpha)
  finalQ <- indY %*%final.q
  lambdabetaz <- finalQ*exp(as.matrix(Z)%*%coef.final)
  goutput <- Gtransform(temp=lambdabetaz, rho=rho, r=r)
  
  temp.loglik <- rep(NA, n)
  for(kk in 1:n){
    if(Delta[kk]==1){
      temp.loglik[kk] <- log(final.q[kk]) + as.matrix(Z[kk,])%*%coef.final + log(goutput$dG[kk]) - goutput$G[kk]
    }else{
      temp.loglik[kk] <- -goutput$G[kk]
    }
  }
  
  loglik.value <- sum(temp.loglik)
  return(loglik.value)
